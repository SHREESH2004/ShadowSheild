...
export async function getRequestsPerMinute(ip: string): Promise<number> {
  return await redisClient.zCard(`ip:timeline:${ip}`);
}

export async function getErrorRate(ip: string): Promise<number> {
...
export async function getTotalBytesMB(ip: string): Promise<number> {
  const stats = await redisClient.hGetAll(`ip:stats:${ip}`);
  const bytes = parseInt(stats.totalBytes || "0");
  return bytes / (1024 * 1024);
}

export async function getEndpointEntropy(ip: string): Promise<number> {
...
export async function getRepeatRate(ip: string): Promise<number> {
...
export async function getCVGap(ip: string): Promise<number> {
  const timestamps = await redisClient.zRangeWithScores(
    `ip:timeline:${ip}`,
    0,
    -1
  );
